package com.scp.scpmadness.item.custom;
...
public class PainiteBowItem extends BowItem implements IVanishable {
        public PainiteBowItem(Item.Properties p_i48522_1_) {
            super(p_i48522_1_);
        }
                    ...
                    if (!((double)f < 0.1D)) {
                        ...
                        if (!p_77615_2_.isClientSide) {
                            ...
                            if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.FLAMING_ARROWS, p_77615_1_) > 0) {
                                abstractarrowentity.setSecondsOnFire(150);
                            }
        ...
        public static float getPowerForTime(int p_185059_0_) {
            float f = (float)p_185059_0_ / 5.0F;
            f = (f * f + f * 2.0F) / 3.0F;
            if (f > 1.0F) {
                f = 1.0F;
            }
